"""
cast_lead / rating / release_date 보완 파이프라인 (TMDB API)

대상:
  - vod.cast_lead   : 166,159건 전체 NULL → TMDB cast에서 주연 3명 추출
  - vod.rating      : 166,159건 전체 NULL → TMDB release_dates에서 한국 등급
  - vod.release_date: 166,159건 전체 NULL → TMDB에서 개봉/방영일

멱등성:
  - cast_lead IS NULL AND rating IS NULL AND release_date IS NULL 인 경우만 처리
  - 재실행 시 이미 채워진 건 자동 스킵
...
def fetch_unprocessed(batch_size: int = 500) -> list[dict]:
    """cast_lead / rating / release_date 모두 NULL인 VOD 조회"""
...
def update_vod(full_asset_id: str, cast_lead, rating, release_date):
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                UPDATE vod
            ...
            """, (cast_lead, rating, release_date, full_asset_id))
...
def run():
    ...
    logger.info("=== cast_lead / rating / release_date 보완 시작 ===")
    ...
    while True:
        ...
        for vod in vods:
            try:
                ...
                update_vod(vod["full_asset_id"], cast_lead, rating, release_date)
                total_processed += 1
