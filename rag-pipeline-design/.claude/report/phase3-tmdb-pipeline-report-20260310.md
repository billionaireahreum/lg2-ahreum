...
| 전체 처리 시리즈 | **20,115개** |
| TMDB 발견 | 4,335개 (**48.6%**) |
| TMDB 미발견 | 4,580개 (51.4%) |
| cast_lead 채움 | 4,043 / 4,335 = **93.3%** |
| rating(KR+US) | 3,037 / 4,335 = **70.1%** |
| release_date 채움 | 4,306 / 4,335 = **99.3%** |
| 총 API 호출 | **17,970회** |
...
## 8. NULL 잔량 집계 결과 (2026-03-10 완료 후)

### ct_cl 실제 값 확인
DB 내 `ct_cl` 실제 값: `TV 연예/오락` (공백+슬래시 포함) — 파이프라인은 정상 처리됨, 집계 쿼리에서만 이름 불일치 확인.

### 5개 ct_cl 전체 현황 (row 기준, tmdb_미처리=0 전체 확인 ✅)

| ct_cl | 전체 row | TMDB 발견률 | rating NULL | cast_lead NULL | cast_guest NULL | date NULL |
|-------|---------|------------|------------|----------------|-----------------|-----------|
| TV드라마 | 67,126 | **76.1%** | 29,505 (43.9%) | 16,583 (24.7%) | 17,340 (25.8%) | 16,087 (24.0%) |
| 영화 | 17,979 | **65.2%** | 8,850 (49.2%) | 6,443 (35.8%) | 6,858 (38.1%) | 6,272 (34.9%) |
| TV 연예/오락 | 19,141 | **57.9%** | 11,559 (60.4%) | 9,605 (50.2%) | 12,972 (67.8%) | 8,060 (42.1%) |
| TV애니메이션 | 25,402 | **34.8%** | 17,974 (70.8%) | 17,186 (67.7%) | 18,213 (71.7%) | 16,550 (65.2%) |
| 키즈 | 20,719 | **24.4%** | 17,040 (82.2%) | 18,649 (90.0%) | 19,199 (92.7%) | 15,710 (75.8%) |

### 잔여 NULL 원인 분석
- 키즈/TV애니메이션: TMDB 미수록 한국 콘텐츠 비율 높음 → NULL 다수
- 영화: 발견률 65%로 양호하나 rating NULL 49%는 TMDB KR 미등록 외화 多
- TV드라마: 발견률 76%로 가장 높음, cast_lead NULL 25%는 단막극/지역방송 위주

### 다음 단계 결정
rule_based 보완 및 KMDB 정교화 **건너뜀** → **임베딩 재생성으로 즉시 진행**
- 현재 메타데이터 수준으로 임베딩 품질 충분 판단
- rating NULL은 임베딩 입력 텍스트 미포함 (필터링 전용) → 임베딩에 영향 없음
...
| 순서 | 작업 | 파일 | 상태 |
|------|------|------|------|
| 1 | 03번 완료 | `03_fill_cast_rating_date.py` | ✅ 완료 (21:12) |
| 2 | NULL 잔량 집계 | SQL 집계 | ✅ 완료 |
| 3 | 임베딩 재생성 | `02_generate_embeddings.py` 수정 | 🔄 진행 예정 |
| 4 | rule-based rating 보완 | 별도 스크립트 | ⏸ 보류 |
| 5 | KMDB 정교화 | TV드라마 한정 | ⏸ 보류 |

### 임베딩 재생성 계획
- 시리즈 단위로 임베딩 연산 후 같은 시리즈 row 전체에 복사
- 입력 텍스트: `제목 / 유형 / 장르 / 감독 / 주연(cast_lead) / 조연(cast_guest) / 줄거리 / 개봉연도`
- rating은 임베딩 본문 제외 (필터링 전용)
- cast_guest 이번엔 반드시 포함 (기존 임시본에서 누락됐던 항목)
