...
| NULL | 모든 컬럼 0 | ✅ |
| 벡터 차원 | 384 | ✅ |
| magnitude mean | 1.000000 (std 3.9e-08) | ✅ 완전 정규화 |
| 유사도 검색 | 겨울왕국 Top10 정상 반환 | ✅ |

### 유사도 검색 특이사항

겨울왕국 Top10이 **전부 겨울왕국 row**로 채워짐.
- 원인: 동일 시리즈 → 같은 벡터 → cosine 유사도 1.0 동률
- 대응: DB 적재 후 추천 검색 시 `DISTINCT ON (normalized_title)` 또는 시리즈 대표 1개만 검색 대상에 포함시키는 로직 필요
- **Parquet 자체는 정상**, 추천 레이어에서 처리할 사안
